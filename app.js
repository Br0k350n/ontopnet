...
app.get('/latest', async (req, res) => {
  try {
    // Query the database for the 3 most recently added servers
    const [recentServers] = await pool.query(`
      SELECT * FROM servers ORDER BY created_at DESC LIMIT 3
    `);

    // Use fetchServerDetails to get additional details for each server
    const serversWithDetails = await Promise.all(
      recentServers.map(async (server) => {
        const details = await fetchServerDetails(server.ip);
        return {
          ...server,
          ...details, // Merge fetched details into the server object
        };
      })
    );
    ...
    const blogs = [
      {
        id: 1,
        title: "Top Tips for FiveM Server Owners",
        content: "Discover how to optimize your FiveM server with these essential tips.",
        created_at: new Date().toISOString(),
      },
      {
        id: 2,
        title: "FiveM Community Spotlight",
        content: "Highlighting the most active and creative servers in the community.",
        created_at: new Date().toISOString(),
      },
      {
        id: 3,
        title: "How to Attract Players to Your Server",
        content: "Strategies for growing and engaging your server community.",
        created_at: new Date().toISOString(),
      },
    ];

    // Render the 'latest' view with the servers and blog posts
    res.render('latest', {
      session: req.session,
      servers: serversWithDetails,
      blogs, // Include the blog posts in the view
    });
  } catch (err) {
    console.error('Error retrieving latest servers:', err);
    res.status(500).send('Error retrieving latest servers');
  }
});
...
app.get('/register', (req, res) => {
  ...
  res.render('register', { experimental: true, verificationToken, session: req.session });
});


app.post('/register-server', async (req, res) => {
  const { ip, verificationCode } = req.body;
  const discordId = req.session.user.discord_Id;
  
  try {
    if (process.env.EXPERIMENTAL_MODE === 'true') {
      // Experimental method: use the current verification process
      const server = await fetchServerDetails(ip);
      const serverVerificationCode = server.citySecret;
      console.log(server.citySecret)

      if (serverVerificationCode === verificationCode) {
        // Verification successful, proceed to register the server
        await pool.query(
          'INSERT INTO servers (ip, verification_code, owner, rank) VALUES (?, ?, ?, LAST_INSERT_ID())',
          [ip, verificationCode, discordId]
        );

        // Update the ranking based on the votes
        await pool.execute(
          `UPDATE servers s1
           JOIN (
             SELECT id, RANK() OVER (
               ORDER BY 
                 CASE
                   WHEN membership = 'premiumplus' THEN 1
                   WHEN membership = 'premium' THEN 2
                   ELSE 3
                 END,
                 votes DESC,
                 RAND()  -- Randomize tie-breakers
             ) AS rank
             FROM servers
           ) s2 ON s1.id = s2.id
           SET s1.rank = s2.rank;`
        ...
        res.redirect('/user/cities');
      } else {
        res.json({ success: false, message: 'Verification failed. Please check the verification code in your server.cfg file.' });
      }
    } else {
      // Placeholder for future verification method
      res.json({ success: false, message: 'Experimental mode is off. Please use the standard verification method.' });
    }
  } catch (error) {
    console.error('Error verifying server:', error);
    res.status(500).send('Error verifying server');
  }
});

app.get('/user/cities', isAuthenticated, async (req,res) => {
  // Show confirmation page to the user
  res.render('confirm-redirect', { 
    session: req.session, 
    user: req.session.user,
    message: 'Are you sure you want to redirect to CityOnTop.com?'
  });
})
...
app.get('/tester-access', async (req, res) => {
  res.render('tester-access')
})
...
app.get('/achievements', isAuthenticated, async (req,res) => {
  const discordId = req.session.user.discord_id; // Use Discord ID from session
  // Fetch user details (including level_data)
  const [userRows] = await pool.query('SELECT * FROM users WHERE discord_id = ?', [discordId]);
  const user = userRows[0];
  const userId = req.session.user.id;
  // Fetch achievements and user's achievement progress
  const [achievements] = await pool.execute(`
    SELECT a.id, a.name, a.description, a.bonus_votes, a.badge, a.required_count, a.isLocked, 
           ua.progress, ua.completed 
    FROM achievements a
    LEFT JOIN user_achievements ua ON a.id = ua.achievement_id AND ua.user_id = ?
    ORDER BY a.isLocked ASC, ua.completed DESC, ua.progress DESC
  `, [userId]);  
...
app.get('/user/dashboard', isAuthenticated, async (req, res) => {
  try {
    ...
    // Parse level_data
    let level = 1, exp = 0, expToNextLevel = 100;
    if (user.level_data) {
      try {
        const levelData = JSON.parse(user.level_data);
        level = levelData.level || 1;
        exp = levelData.exp || 0;
        expToNextLevel = levelData.expToNextLevel || 100;
      } catch (error) {
        console.error("Error parsing level_data:", error);
      }
    }
    ...
    // Fetch voting history (join with servers to get details)
    const [voteRows] = await pool.query(`
      SELECT sv.id AS server_id, sv.ip, sv.rank, sv.membership, uv.server_name, uv.vote_time
      FROM user_votes uv
      JOIN servers sv ON uv.server_id = sv.id
      WHERE uv.user_id = ?
    `, [discordId]);
    const votes = voteRows;
    const totalVotes = votes.length;
    const lastVote = votes[votes.length - 1];
    const nextVoteTime = lastVote ? new Date(lastVote.vote_time).setHours(24, 0, 0, 0) : Date.now();

    // ---- QUEST FUNCTIONALITY ----

    // Fetch available quests (including reward_xp)
    const [quests] = await pool.query("SELECT id, name, reward_xp FROM quests");
    ...
    // Categorize quests into active and completed
    let completedQuests = [];
    let activeQuests = [];
    quests.forEach(quest => {
      const userQuest = userQuests.find(q => q.quest_id === quest.id);
      if (userQuest && userQuest.status === 'completed') {
        completedQuests.push({ ...quest, reward_xp: quest.reward_xp });
      } else {
        activeQuests.push({ ...quest, reward_xp: quest.reward_xp });
      }
    });

    // ---- END QUEST FUNCTIONALITY ----

    // Fetch sponsorship details
    const [sponsorRows] = await pool.query('SELECT * FROM sponsor_requests WHERE email = ? AND status = ?', [user.email, 'accepted']);
    const sponsor = sponsorRows.length > 0 ? sponsorRows[0] : null;
...
app.post('/sponsor-request', async (req, res) => {
  try {
    // Destructure all the form fields
    const { name, email, company, message, industry, product, imgUrl } = req.body;
    ...
    // Check if a request already exists for this email or IP
    const [existing] = await pool.query(
      `SELECT * FROM sponsor_requests WHERE email = ? OR ip = ?`,
      [email, ip]
    );
    if (existing.length > 0) {
      return res.status(400).send('You have already submitted a sponsor request.');
    }

    // Use the provided image URL; the image will be downloaded later upon acceptance.
    const img = imgUrl || null;

    // Insert the sponsor request into the database with a default status of "pending"
    // Assumes your sponsor_requests table has columns for industry, product, and img.
    const [result] = await pool.query(
      `INSERT INTO sponsor_requests (name, ip, email, company, industry, product, message, img, status, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', NOW())`,
      [name, ip, email, company, industry, product, message, img]
    );
    console.log(`Sponsor request inserted with ID: ${result.insertId}`);
...
app.get('/user/settings', isAuthenticated, async (req, res) => {
  try {
    ...
    const [userRows] = await pool.query('SELECT * FROM users WHERE discord_id = ?', [discordId]);
    const user = userRows[0];
    res.render('user-settings', { user, session: req.session });
  } catch (err) {
    console.error('Error fetching user data:', err);
    res.status(500).send('Error fetching user data');
  }
...
app.get('/user/achievements', isAuthenticated, async (req, res) => {
  const userId = req.session.user.id;

  // Get all achievements and the user's progress
  const [achievements] = await pool.execute(`
    SELECT a.id, a.name, a.description, a.bonus_votes, a.badge, a.required_count, 
           ua.progress, ua.completed 
    FROM achievements a
    LEFT JOIN user_achievements ua ON a.id = ua.achievement_id AND ua.user_id = ?
  `, [userId]);

  // Calculate total and earned achievements
  const totalAchievements = achievements.length;
  const earnedAchievements = achievements.filter(a => a.completed).length;

  // Render the achievements page with the progress info
  res.render('achievements', { 
    session: req.session, 
    achievements, 
    earnedAchievements, 
...
app.post('/user/settings', isAuthenticated, async (req, res) => {
  ...
  let avatarUrl = req.user.avatar;  // Default to the existing avatar
  if (avatar && avatar !== req.user.avatar) {
    avatarUrl = avatar;  // Use the provided URL for the avatar
  }
  ...
  // Remove the trailing comma from the query if necessary
  query = query.replace(/,$/, '');

  // Always include the Discord ID for the WHERE clause
  query += ' WHERE discord_id = ?';
  params.push(discordId);

  // Execute the query and handle the response
  try {
    if (params.length > 0) {  // Proceed only if there are fields to update
...
app.get('/ads/:section', async (req, res) => {
  try {
      const section = req.params.section;
      const [ads] = await pool.execute("SELECT * FROM ads WHERE section = ?", [section]);
